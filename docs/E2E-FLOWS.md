...
3. Scroll to **Extracted draft**: fields are pre-filled with `AI` badges + a confidence score, and the **AI Bill Review** panel lists anomalies (e.g. *"Fuel surcharge 32% above 6-month average"*) with severity + **Accept / Dismiss**.
4. **Save draft** → persists a real bill (line items + flags) and opens its cockpit.

*Expected:* a real draft bill is created and reachable in **Bills → Drafts**.
...
*Expected:* the open-flag count updates. (The AI review is visible here too, not only on Capture.)
...
## 11. Expense vs. item line coding
...
## 12. Line-item splits + allocation templates
...
## 14. Payments (schedule / mark paid)
...
## 15. Payment-failed recovery card
...
## 20. Command palette, shortcuts & dark mode
